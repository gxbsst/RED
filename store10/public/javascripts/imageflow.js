...
function ImageFlow ()
{
	...
	this.defaults =
	{
		...
		reflectionP:        0.4,            /* Height of the reflection in % of the source image */
		...
		sliderWidth:          10,             /* Width of the slider in px */
...
/*        var height = Math.round(width * 0.1085);*/
	...
	/*改变container的高度*/
		/*this.maxHeight = Math.round(thisObject.iWidth * 0.5);*/
		this.maxHeight = Math.round(thisObject.iWidth * 0.2);
		
		this.maxFocus = thisObject.imageFocusMax * thisObject.xStep;
/*    this.size = thisObject.iWidth * 0.5;*/
/*改变当前image的大小*/
		this.size = thisObject.iWidth * 0.54;
		
/*    this.scrollbarWidth = Math.round(thisObject.iWidth * 0.6);*/

    /* 修改scrollbar的宽度*/
		this.scrollbarWidth = Math.round(thisObject.iWidth * 0.998);
		...
		thisObject.imagesDiv.style.height = Math.round(thisObject.iWidth * 0.138) + 'px';
		...
		thisObject.scrollbarDiv.style.marginLeft = Math.round(thisObject.iWidth * 0.05) + 'px';
	...
	this.moveTo = function(x)
	{
		...
		for (var index = 0; index < thisObject.max; index++)
		{
			...
			else 
			{
				...
				/*设置图片的top*/
				var newImageTop = (thisObject.iWidth * 0.14 - newImageH) + ((newImageH / (thisObject.reflectionP + 1)) * thisObject.reflectionP);

				/* Set new image properties */
				/*调整这个参数可以校正图片离左边，右边的距离*/
				...
				switch ( image.i == thisObject.imageID )
				{
					case false:
						...
						/*description = document.getElementById('description' + thisObject.imageID);
              ...
              /* image.onclick = thisObject.onClick;*/
						...
						if ( thisObject.ImageFlowID == "myImageFlow") {
							decOuter = document.getElementById("description-outer");
							for(i = 0; i < decOuter.childNodes.length; i++) {
								if ( decOuter.childNodes[i].nodeType == 1) {
									decOuter.childNodes[i].style.display = 'none';
								}
							}
							description = document.getElementById('description' + thisObject.imageID);
...
domReady(function()
{
  ...
  var instanceOne1 = new ImageFlow();
  instanceOne1.init({ ImageFlowID:'myImageFlow1', 
  reflections: false, 
  reflectionP: 0.0,
  startID: 3,
  /*                    imageFocusMax: 1*/
