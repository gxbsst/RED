module Admin::BaseHelper
  ...
  # Rendering navigator tab
  def navigator_tab( display_name, url_params)
    return unless @current_user.accessible_to?(url_params)
    
    list = %Q(<li class="#{ @cname == %Q(#{url_params[:controller].split('/').last}) ? 'active' : ''} ">)
    list << link_to_section("#{display_name}", url_params)
    list << "</li>"
  end
end
