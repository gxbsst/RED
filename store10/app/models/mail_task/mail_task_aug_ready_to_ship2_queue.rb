class MailTaskAugReadyToShip2QueueLineItem < ActiveRecord::Base
  include MailTask::General
  belongs_to :mail_task_aug_ready2_to_ship_queue
end

class MailTaskAugReadyToShip2Queue < ActiveRecord::Base
  belongs_to :mail_task
  include MailTask::General
  has_many :mail_task_aug_ready_to_ship2_queue_line_items, :dependent => :destroy
    Delivery_range_begin = 2651
    Delivery_range_end   = 2900
          ...
          [
            ...
            [
              ...
              [ 'CU 0102176', 'BrianB']
                ...
                # Check the the email address of the customer service...
                ...
                if account
                  # Check the customer service have been assiged and log them...
                  if (!account.assign_to.blank? and account.assign_to != i.last)
                    open("#{RAILS_ROOT}/log/assign_to.log", "a+") { |file| file << "#{account.ax_account_number}\t#{account.assign_to}\t#{i.last}\n" } 
                  end
                  if account.assign_to.blank? || account.assign_to == 'Nate' || account.assign_to == 'Sean'
...
# ~> -:509: syntax error, unexpected kEND, expecting $end
