class MailTaskFebReadyToShipQueueLineItem < ActiveRecord::Base
  include MailTask::MailDB
  belongs_to :mail_task_feb_ready_to_ship_queue
end

class MailTaskFebReadyToShipQueue < ActiveRecord::Base
  include MailTask::General
  
  belongs_to :mail_task
  has_many :mail_task_feb_ready_to_ship_queue_line_items, :dependent => :destroy
  ...
  class << self
    def init_queues
      ...
      [
        'TRUNCATE mail_task_feb_ready_to_ship_queues',
        'TRUNCATE mail_task_feb_ready_to_ship_queue_line_items'
      ].each do |statement|
    ...
    def create_queue(account)
      ...
      account.ax_orders.map(&:ax_order_line_items).flatten.each do |line_item|
        ...
        line_items << MailTaskFebReadyToShipQueueLineItem.new(
      ...
      queue.mail_task_feb_ready_to_ship_queue_line_items = line_items
  ...
  def line_items
    @line_items ||= mail_task_feb_ready_to_ship_queue_line_items
  end
