class MailTaskJanRedoneDeliveryPolicySixToFiveHundredQueue < ActiveRecord::Base
  ...
  class << self
    def init_queues
      ...
      AxOrderLineItem.find(:all, :conditions => [
          'item_id = ? and sales_item_reservation_number in (?) and invoiced_in_total is ?',
  ...
  def send_mail
    options = {
      :subject => "RED ONE Delivery Policy",
