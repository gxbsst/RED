class MailTaskMarNearlyReadyToShipQueueLineItem < ActiveRecord::Base
  include MailTask::MailDB
  belongs_to :mail_task_mar_nearly_ready_to_ship_queue
end

class MailTaskMarNearlyReadyToShipQueue < ActiveRecord::Base
  include MailTask::General
  
  belongs_to :mail_task
  has_many :mail_task_mar_nearly_ready_to_ship_queue_line_items, :dependent => :destroy
