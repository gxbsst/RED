class MailTaskJanPrepareDeliveryQueueLineItem < ActiveRecord::Base
  include MailTask::MailDB
  belongs_to :mail_task_jan_prepare_delivery_queue
end

class MailTaskJanPrepareDeliveryQueue < ActiveRecord::Base
  include MailTask::General
  
  belongs_to :mail_task
  has_many :mail_task_jan_prepare_delivery_queue_line_items, :dependent => :destroy
  ...
  class << self
    def init_queues
      ...
      [
        'TRUNCATE mail_task_jan_prepare_delivery_queues',
        'TRUNCATE mail_task_jan_prepare_delivery_queue_line_items'
      ].each do |statement|
    ...
    def create_queue(account)
      ...
      # TODO: exclude "Delivered" or "Canceled" orders.
      line_items = []
      account.ax_orders.map(&:ax_order_line_items).flatten.each do |line_item|
        ...
        line_items << MailTaskJanPrepareDeliveryQueueLineItem.new(
      ...
      queue.mail_task_jan_prepare_delivery_queue_line_items = line_items
  ...
  def line_items
    @line_items ||= mail_task_jan_prepare_delivery_queue_line_items
  end
